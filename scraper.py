...
def get_build_id(session: requests.Session, player_id: int, slug: str) -> str:
...
def fetch_player_json(session: requests.Session, player_id: int, slug: str) -> dict:
    """Resolve buildId then fetch the _next/data JSON for a player."""
    print(f"→ Resolving buildId for player {player_id} ({slug})...")
    build_id = get_build_id(session, player_id, slug)
...
def fetch_match_json(session: requests.Session, match_url: str) -> dict:
    """
    ...
    if parts[0] == "matches":
        ...
        if not fragment:
            # Try to extract matchId from the page JSON itself as a fallback
            mid_m = re.search(r'"matchId"\s*:\s*(\d+)', resp.text)
...
def main():
    ...
    try:
        raw_data = fetch_player_json(session, args.player_id, args.slug)
